...
#    master.maxsize(width=24*SCALE, height=18*SCALE)
    ...
    def __init__(self):
        ...
        labelText=StringVar()
        labelText.set("Current joint positions")
        ...
        self.StartPublishingJointState=Button(text="Start publishing joint state",command=self.startpub).grid(row=1,column=6)
        self.StopPublishingJointState=Button(text="Stop publishing joint state",command=self.stoppub).grid(row=2,column=6)
        self.validatebutton=Button(text="Validate Transformation ",command=self.validatepositions).grid(row=3,column=6) 
        self.SetDesiredTransform=Button(text="Publish Desired transform",command=self.gototransform).grid(row=4,column=6)
        self.StopDesiredTransform=Button(text="Stop publishing Desired transform",command=self.stoptransform).grid(row=5,column=6)
        self.quitbutton=Button(text="Quit",command=self.exit_gui,bg="red",activebackground="red").grid(row=6,column=6) 
...
#        self.w=Label(self.master,textvariable=self.v,bg="white").grid(row=0,column=6)
    ...
    def make_tf_display(self,i):             
        ...
        Label_in_value_box=Label(self.master, textvariable=self.wTp_label[i],height=1,width=15,bg="white").grid(row=self.r,column=4)        
        return Label_in_value_box     
        
    def make_entry_button_for_transform(self,name): 
        Label_next_to_entry_box=StringVar()
        Label_next_to_entry_box.set(name)
        Label_in_value_box=Label(self.master, textvariable=Label_next_to_entry_box,height=4).grid(row=self.r,column=2)        
        directory=StringVar(None)
        entry=Entry(self.master,textvariable=directory,width=10)
        entry.grid(row=self.r,column=3)   
        return entry 
